...
class KmlParserService {
  ...
  extractGeometry(placemark) {
    if (placemark.Point) {
      ...
      if (coordinates.length > 0) {
        ...
        return { pointType: 'Point', latitude, longitude, altitude, coordinates: { points: coordinates } }
      }
    }
    if (placemark.LineString) {
      ...
      if (coordinates.length > 0) {
        const [longitude, latitude, altitude = 0] = coordinates[0]
        return { pointType: 'LineString', latitude, longitude, altitude, coordinates: { points: coordinates } }
      }
    }
    if (placemark.Polygon) {
      const outerBoundary = placemark.Polygon.outerBoundaryIs?.LinearRing?.coordinates || placemark.Polygon.outerBoundaryIs?.coordinates
      if (outerBoundary) {
        const coordinates = this.parseCoordinates(outerBoundary)
        if (coordinates.length > 0) {
          const center = this.calculatePolygonCenter(coordinates)
          return { pointType: 'Polygon', latitude: center.latitude, longitude: center.longitude, altitude: center.altitude, coordinates: { outer: coordinates, inner: this.extractInnerBoundaries(placemark.Polygon) } }
        }
      }
    }
    return null
  ...
  calculatePolygonCenter(coordinates) {
    ...
    let totalLat = 0, totalLng = 0, totalAlt = 0, count = 0
    for (const c of coordinates) {
      if (c.length >= 2) { totalLng += c[0]; totalLat += c[1]; totalAlt += c[2] || 0; count++ }
    }
    return { longitude: count ? totalLng / count : 0, latitude: count ? totalLat / count : 0, altitude: count ? totalAlt / count : 0 }
  }
  ...
  extractStyleData(placemark) {
    ...
    if (placemark.ExtendedData?.Data) {
      ...
      for (const d of dataArray) { if (d.name && d.value) styleData[d.name] = d.value }
    }
    if (placemark.styleUrl) styleData.styleUrl = placemark.styleUrl
    return styleData
  }
  async validateKmlFile(filePath) {
    try { const xmlContent = await fs.readFile(filePath, 'utf8'); return this.validateKmlString(xmlContent) }
    catch (error) { return { valid: false, error: '无法读取文件: ' + error.message } }
  }
  async validateKmlString(xmlContent) {
    ...
    } catch (error) { return { valid: false, error: 'KML格式错误: ' + error.message } }
  }
}
