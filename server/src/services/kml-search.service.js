    ...
    try {
      ...
      for (const kmlFile of filesToSearch) {
        ...
        if (results.length >= offset + limit) {
          Logger.debug('已收集足够匹配项，终止进一步文件解析', {
      ...
      Logger.debug('KML点位搜索完成', {
        ...
        returned:
          Math.max(0, results.length - offset) > 0
            ? Math.min(limit, Math.max(0, results.length - offset))
            : 0,
  ...
  static async parseKMLFile(kmlFilePath) {
    try {
      ...
      if (cacheEntry && cacheEntry.mtime === mtime && Array.isArray(cacheEntry.points)) {
        Logger.debug('KML解析命中缓存', {
          file: kmlFilePath,
          cachedCount: cacheEntry.points.length,
        });
