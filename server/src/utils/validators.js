...
class Validators {
  ...
  static validateCoordinate(lat, lng) {
    ...
    if (latitude < COORDINATE_BOUNDS.LATITUDE.MIN || latitude > COORDINATE_BOUNDS.LATITUDE.MAX) {
      return {
        valid: false,
        message: `纬度必须在 ${COORDINATE_BOUNDS.LATITUDE.MIN} 到 ${COORDINATE_BOUNDS.LATITUDE.MAX} 之间`,
      };
    }
    ...
    if (
      ...
      return {
        valid: false,
        message: `经度必须在 ${COORDINATE_BOUNDS.LONGITUDE.MIN} 到 ${COORDINATE_BOUNDS.LONGITUDE.MAX} 之间`,
      };
    }
  ...
  static validateIds(ids) {
    ...
    return ids.map((id) => parseInt(id)).filter((id) => !isNaN(id) && id > 0);
  }
