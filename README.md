...
- **用户名认证体验**：支持基于用户名的注册/登录流程，提供“用户名不存在 / 密码错误”精确提示与 Session 自动延续。
- **管理后台快捷操作**：后台头部默认包含“返回首页 / 切换侧栏 / 退出登入”操作，便于快速切换视图与注销账号。
...
- 登录/注册以 `username` 作为唯一标识，前端会过滤首尾空格并在冲突时提示“用户名已存在”。
- 登录失败会区分“用户名不存在”与“密码错误”，避免模糊提示；成功后会刷新 Pinia `auth` store 并注入 Session Cookie。
- 管理后台头部提供显式“退出登入”按钮，调用 `authStore.logout()` 清理状态并跳转至登录页面。
- `start.sh` 启动脚本会检测并迁移 SQLite `sessions` 表结构（包含 `expires_at` 字段与索引），保证 Prisma Session Store 与 Express 会话保持兼容。
