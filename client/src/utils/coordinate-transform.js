...
const EE = Number('0.00669342162296594323'); // 偏心率平方
/**
 * 前端坐标转换工具
 * 支持 WGS84 <-> GCJ02 等转换
 *
 * 说明（维护者注）：
 *  - 地图底图使用高德/GCJ-02 瓦片时，应用中的显示坐标在部分流程中会以 GCJ-02 表示。
 *  - 为了保证导出文件的互操作性（KML/GeoJSON/CSV 等），导出处会将当前被视为 GCJ-02 的坐标转换为 WGS84 写入文件。
 *  - 本文件提供的函数（wgs84ToGcj02 / gcj02ToWgs84 / convertCoordinate 等）是导出/显示边界处的核心转换函数。
 */
