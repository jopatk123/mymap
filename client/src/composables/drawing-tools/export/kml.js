  ...
  wgsDrawings.forEach((drawing, idx) => {
    ...
    if (drawing && drawing.coordinates) {
      if (drawing.type === 'Point' && Array.isArray(drawing.coordinates)) {
        ...
        Array.isArray(drawing.coordinates[0])
      ) {
        ...
        coordsStr = drawing.coordinates
          .map((c) => (Array.isArray(c) ? `${c[1]} ${c[0]}` : ''))
          .join('|');
      }
    }
...
export function exportToKml(format = 'kml') {
  ...
  const wgsDrawings = drawings.value.map((d) => {
    ...
    if (copy.type === 'Point' && Array.isArray(copy.coordinates)) {
      ...
      copy.type === 'Polygon' &&
      Array.isArray(copy.coordinates) &&
      Array.isArray(copy.coordinates[0])
    ) {
  ...
  if (format === 'csv') {
    try {
      ...
      const filename = `地图绘制导出_${new Date()
        .toISOString()
        .slice(0, 19)
        .replace(/:/g, '-')}.csv`;
