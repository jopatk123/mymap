  ...
  const handleClick = (e) => {
    ...
    if (!isDrawing) {
      ...
      L.marker(e.latlng, {
        icon: L.divIcon({
          className: 'measure-marker',
          html: `<div style="background: rgba(255,0,0,0.8); color: white; padding: 2px 6px; border-radius: 4px; font-size: 12px;">${totalDistance < 1000 ? Math.round(totalDistance) + 'm' : (totalDistance / 1000).toFixed(2) + 'km'}</div>`,
          iconSize: [80, 20],
          iconAnchor: [40, 10]
        })
      }).addTo(mapInstance.drawingLayerGroup)
