...
export function useKMLBaseMap() {
  ...
  const deleteKMLFile = async (fileId, fileName) => {
    try {
      await ElMessageBox.confirm(`确定要删除KML文件"${fileName}"吗？此操作不可恢复。`, '确认删除', {
  ...
  const fileStatistics = computed(() => {
    const files =
      store.kmlFiles && store.kmlFiles.value ? store.kmlFiles.value : store.kmlFiles || [];
    const totalFiles = Array.isArray(files) ? files.length : files?.length || 0;
    const totalPoints =
      store.totalPointsCount && store.totalPointsCount.value ? store.totalPointsCount.value : 0;
    const visiblePoints =
      store.visiblePointsCount && store.visiblePointsCount.value
        ? store.visiblePointsCount.value
        : 0;
  ...
  return {
    ...
    kmlPoints: computed(() =>
      store.kmlPoints && store.kmlPoints.value ? store.kmlPoints.value : []
    ),
    visiblePoints: computed(() =>
      store.visiblePoints && store.visiblePoints.value ? store.visiblePoints.value : []
    ),
    loading: computed(() =>
      store.loading && typeof store.loading.value !== 'undefined' ? store.loading.value : false
    ),
    uploadDialogVisible,
    uploading,
