  ...
  const removeMarker = (id) => {
    ...
    if (markerIndex > -1) {
      ...
      try {
        ...
        if (useCluster) {
      const group = type === 'video' ? clusterManager.videoClusterGroup : clusterManager.panoramaClusterGroup;
  ...
  const fitBounds = () => {
    ...
    try {
      ...
      if (validMarkers.length === 0) {
        let bounds = null
        if (clusterManager.videoClusterGroup && map.value.hasLayer(clusterManager.videoClusterGroup) && typeof clusterManager.videoClusterGroup.getBounds === 'function') {
          const b = clusterManager.videoClusterGroup.getBounds()
          if (b && b.isValid && b.isValid()) bounds = bounds ? bounds.extend(b) : b
        }
        if (clusterManager.panoramaClusterGroup && map.value.hasLayer(clusterManager.panoramaClusterGroup) && typeof clusterManager.panoramaClusterGroup.getBounds === 'function') {
