...
const setSearchMarker = (lat, lng, labelOrPoint = '搜索位置') => {
  ...
  searchMarker = L.marker([lat, lng], { title: typeof labelOrPoint === 'string' ? labelOrPoint : (labelOrPoint.name || '搜索位置') })
  ...
  if (labelOrPoint && typeof labelOrPoint === 'object') {
    ...
    const amapUrl = `https://uri.amap.com/marker?position=${lngStr},${latStr}&name=${encodeURIComponent(name)}`
    const baiduUrl = `https://api.map.baidu.com/marker?location=${latStr},${lngStr}&title=${encodeURIComponent(name)}&content=${encodeURIComponent(source)}&coord_type=bd09ll&output=html`

    const popupHtml = `
      <div style="max-width: 240px;">
        <h4 class="popup-title">${escapeHtml(name)}</h4>
        <p class="popup-meta">${escapeHtml(desc)} ${source ? `<span class=\"popup-source\">[${escapeHtml(source)}]</span>` : ''}</p>
    ...
    // 简单文本弹窗
    const label = typeof labelOrPoint === 'string' ? labelOrPoint : (labelOrPoint && labelOrPoint.name) || '搜索位置'
