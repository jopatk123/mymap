    ...
    >
      <div v-if="selectedFile" class="file-details-content">
        ...
        <div v-if="filePoints.length > 0" class="points-preview">
          <el-table :data="filePoints.slice(0, 10)" size="small" max-height="300">
            ...
            <el-table-column prop="altitude" label="海拔" width="80">
              <template #default="{ row }"> {{ row.altitude || 0 }}m </template>
            </el-table-column>
          </el-table>
      ...
      <template #footer>
        ...
        <el-button v-if="selectedFile" type="primary" @click="downloadFile(selectedFile)">
          下载文件
        </el-button>
      </template>
    </el-dialog>
...
watch(selectedFile, async (newVal) => {
  ...
  try {
    ...
    // suppressed error logging in UI component
  }
});
